...
export default function OrderForm() {
  ...
  const [deliveryDate, setDeliveryDate] = useState('') // kosong = hari ini
  const [addonProducts, setAddonProducts] = useState([]) // produk add-on
  const [selectedAddons, setSelectedAddons] = useState({}) // {product_id: qty}
  ...
  const regularProducts = availableProducts.filter(p => p.price > 0 && !p.name.includes('(Free)') && !isAddon(p.name))
  ...
  const total = regularTotal + promoTotal + addonTotal
  ...
  const checkDuplicate = async () => {
    ...
    const normalName = normalizeName(info.name).toLowerCase().trim()

    // Cari customer dengan NAMA YANG SAMA PERSIS (tapi mungkin gedung beda)
    const { data: existing } = await supabase
      .from('customers')
      .select('id, name, phone, gedung, lantai, total_orders, total_spent')
      .ilike('name', normalName) // exact match case-insensitive
      .limit(10)

    if (!existing || existing.length === 0) { setStep(3); return }

    // Cari yang gedungnya BERBEDA dari input sekarang
    const samaNamaBedaGedung = existing.find(c => {
      const cGedung = (c.gedung || '').toLowerCase().replace(/\s/g,'').slice(0,6)
      const inGedung = info.gedung.toLowerCase().replace(/\s/g,'').slice(0,6)
      return cGedung !== inGedung // gedung beda = mungkin pindah gedung
    })

    if (samaNamaBedaGedung) {
      setDupCandidate({ existing: samaNamaBedaGedung, incoming: { name: normalName, gedung: info.gedung, lantai: info.lantai } })
      return
    }
  ...
  const submitOrder = async () => {
    ...
    try {
      ...
      // Add-on items
      const addonOrderItems = Object.entries(selectedAddons)
        .filter(([, qty]) => qty > 0)
        .map(([id, qty]) => {
          const prod = addonProducts.find(p => p.id === id)
          return {
            order_id: order.id,
            product_id: id,
            product_name: prod?.name || 'Add-on',
            quantity: qty,
            price: prod?.price || 0,
            subtotal: (prod?.price || 0) * qty,
          }
        })
      const allItems = [...regularOrderItems, ...promoOrderItems, ...freeOrderItems, ...addonOrderItems]
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1A2E0A 0%, #2D5016 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div style={{ background: '#fff', borderRadius: 20, padding: '2rem', maxWidth: 420, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        <div style={{ textAlign: 'center', marginBottom: 20 }}>
          ...
          <p style={{ color: '#666', fontSize: 13, marginTop: 6 }}>Pelanggan dengan nama yang sama ditemukan di lokasi berbeda — apakah orang yang sama?</p>
        </div>
        ...
        <p style={{ textAlign: 'center', fontSize: 14, color: '#555', marginBottom: 16 }}>
          Apakah <strong>{dupCandidate.existing.name}</strong> di <strong>{dupCandidate.existing.gedung}</strong> adalah orang yang sama dengan pemesan sekarang di <strong>{dupCandidate.incoming.gedung}</strong>?
        </p>
  ...
  if (step === 4) return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1A2E0A 0%, #2D5016 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div style={{ background: '#fff', borderRadius: 20, padding: '2rem', textAlign: 'center', maxWidth: 400, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
        ...
        <button onClick={() => { setStep(1); setCart({}); setPromoCart([]); setFreeItems([]); setDupChecked(false); setDupCandidate(null); setDeliveryDate(''); setSelectedAddons({}); setInfo({ name: '', gedung: '', lantai: '', phone: '', catatan: '' }); setOrderNum(''); setWaUrl('') }}
          ...
          <div style={{ background: '#fff', borderRadius: 20, padding: '1.5rem', boxShadow: '0 10px 40px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {Object.entries(selectedAddons).filter(([,q]) => q > 0).map(([id, qty]) => {
                ...
                if (!prod) return null
                return (
                  <div key={id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #F0EDE8', fontSize: 14 }}>
                    <span>➕ {prod.name.replace(/add-on[:\s]*/i, '')} <span style={{ color: '#888' }}>x{qty}</span></span>
                    <span style={{ fontWeight: 600 }}>{formatHarga(prod.price * qty)}</span>
                  </div>
                )
              })}
            ...
            {addonProducts.length > 0 && (
              <div style={{ background: '#FFF9F0', border: '2px solid #E8A838', borderRadius: 14, padding: '14px', marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 14, color: '#1A2E0A' }}>Mau tambah sesuatu?</div>
                    <div style={{ fontSize: 12, color: '#888' }}>Tambahan pilihan untuk melengkapi pesananmu</div>
                  </div>
                </div>
                {addonProducts.map(p => {
                  ...
                  return (
                    <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid #F0EDE8' }}>
                      <div>
                        <div style={{ fontWeight: 600, fontSize: 14 }}>{p.name.replace(/add-on[:\s]*/i, '')}</div>
                        <div style={{ fontSize: 13, color: '#E8A838', fontWeight: 700 }}>+{formatHarga(p.price)}</div>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        {qty === 0 ? (
                          <button onClick={() => setSelectedAddons(a => ({ ...a, [p.id]: 1 }))}
                            style={{ padding: '7px 16px', background: '#E8A838', color: '#fff', border: 'none', borderRadius: 20, fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
                            + Tambah
                          </button>
                        ) : (
                          <>
                            <button onClick={() => setSelectedAddons(a => ({ ...a, [p.id]: Math.max(0, (a[p.id]||0) - 1) }))}
                              style={{ width: 30, height: 30, borderRadius: '50%', border: '2px solid #E8A838', background: '#fff', color: '#E8A838', fontWeight: 700, cursor: 'pointer', fontSize: 16 }}>−</button>
                            <span style={{ fontWeight: 700, minWidth: 20, textAlign: 'center' }}>{qty}</span>
                            <button onClick={() => setSelectedAddons(a => ({ ...a, [p.id]: (a[p.id]||0) + 1 }))}
                              style={{ width: 30, height: 30, borderRadius: '50%', border: 'none', background: '#E8A838', color: '#fff', fontWeight: 700, cursor: 'pointer', fontSize: 16 }}>+</button>
                          </>
                        )}
                      </div>
                    </div>
                ...
                {addonTotal > 0 && (
                  <div style={{ marginTop: 10, fontSize: 13, color: '#E8A838', fontWeight: 700, textAlign: 'right' }}>
                    Total add-on: +{formatHarga(addonTotal)}
                  </div>
                )}
              </div>
            )}
